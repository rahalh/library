namespace Media.Test.IntegrationTests
{
    ...
    public class MediaAPITest : IClassFixture<IntegrationTestFactory>, IAsyncLifetime
    {
        ...
        [InlineData(2)]
        [InlineData(5)]
        public async Task ListMedia_WhenPageSizeProvided_ReturnsMediaList(int pageSize)
        {
            var resp = await this.httpClient.GetAsync($"api/media?pageSize={pageSize}");
            var bodyJSON = await resp.Content.ReadAsStringAsync();
            ...
            {
                case < 4:
                    media.Items.Count.ShouldBe(pageSize);
                    break;
                case > 4:
                    media.Items.Count.ShouldBe(4);
                    break;
